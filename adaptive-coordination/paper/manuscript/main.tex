...
linear-regime conditions
($\omega \in \{0.5, 1.5, 5.0\}$, $\Tcorr \in \{100, 33, 10\}$
steps), $\Lopt$ lies on the line $L = 0.29\,\Tcorr$ (Panel~B),
and $\omdt\cdot\Lopt \in \{0.30, 0.21, 0.30\}$ (Panel~C).
This suggests the reward-optimal shared-arm window follows the same
$1/(\omdt)$ scaling as the pre-registered threshold $\Lmax$, at
roughly the same constant.  That coincidence is not required by
theory — $\Lopt$ and $\Lmax$ measure different things — and should
be treated as suggestive rather than confirmatory.
...
rapid-rotation condition ($\omega{=}8.0$) gives a noise-dominated
value.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_gamma_sweep}
\Description{Three-panel figure. Left: shared-arm reward vs L for seven omega values, coloured by regime (blues for linear, other colours for anomalous). Centre: L-opt vs T-corr scatter with a linear fit through origin (slope 0.29) through the three linear-regime points. Right: omega-dt times L-opt vs omega scatter showing the three linear-regime points near 0.30 and the anomalous conditions above.}
...
blue) peak at $L$ values consistent with $\omdt\cdot\Lopt{\approx}0.30$;
anomalous conditions (dashed) are flat or boundary-limited.
...
\textbf{Right:} $\omdt\cdot\Lopt$ vs.\ $\omega$.
Linear-regime points cluster near $0.30$ (dashed band:
${\pm}20\%$); anomalous conditions lie above.  Coincidence with
the pre-registered $c{\approx}0.30$ is suggestive but not
confirmatory — $\Lopt$ and $\Lmax$ are different quantities.}
