...
runs show the mechanism's predicted directional signatures: a
U-shaped error profile at slow and increasing error beyond $\Lmax$
at mid (adjacent paired $t$-tests: $t(31){\geq}3.40$, $p{<}0.001$
at both speeds), and inter-agent angular dispersion structurally zero
in the shared arm and non-zero in the independent arm.  The coordination benefit landscape under rotation is
...
\begin{enumerate}
  ...
  failure mechanism: instrumented runs simultaneously show condition-specific
  shared-arm angular error profiles and inter-agent
  ...
  At slow ($\omega{=}1.5$), shared-arm angular error is U-shaped with
  an observed minimum at $\Lmax$.
  At mid ($\omega{=}5.0$), the lowest error among the instrumented
  windows occurs at $\Lmax{=}3$, after which error increases.
...
\begin{enumerate}
  \item \textbf{Shared-arm angular error is lowest at $\Lmax$ in both conditions.}
  \emph{Slow ($\omega{=}1.5$, U-shaped):} mean angular error falls from
  ...
  \emph{Mid ($\omega{=}5.0$):} the instrumented windows are
  $L{\in}\{3,10,30,45\}$; $L{=}3{=}\Lmax$ is the left boundary, so the
  data show only that error increases after $\Lmax$.  Error rises from
  0.471~rad at $L{=}3$ to 0.538 at $L{=}10$ and 1.215 at $L{=}30$ as
  sinc attenuation dominates beyond the linear-rotation regime.
  Paired comparison ($r{\approx}0.51$):
...
\Description{Four-panel figure arranged in two rows. Top row: slow condition (omega=1.5). Bottom row: mid condition (omega=5.0). Left column: mean angular error vs L for shared (blue circles) and independent (red squares) arms with SE bands and predicted lag dotted line. Right column: inter-agent dispersion vs L. Top row (slow): shared arm shows a U-shaped error with minimum at Lmax=10; bottom row (mid): error is lowest at Lmax=3 and rises thereafter (left boundary, U-shape not established from data). Shared arm dispersion is zero throughout both conditions; independent arm dispersion is non-zero and decreases with L.}
...
Shared arm (blue): at slow ($\omega{=}1.5$) error is U-shaped with
observed minimum at $\Lmax{=}10$, confirmed by adjacent paired $t$-tests
($t(31){\in}\{8.40,\,3.40\}$, both $p{<}0.001$; $B{=}10{,}000$ bootstrap
CIs exclude zero).
At mid ($\omega{=}5.0$) the lowest error among instrumented windows
occurs at $\Lmax{=}3$, after which error rises steeply; $L{=}3$ is the
left boundary so a full U-shape is not established from these data
($t(31){=}5.30$, $p{<}0.001$ for $E(10){>}E(3)$).
