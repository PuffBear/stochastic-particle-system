...
The irregular pattern suggests the decay controller's crossover
depends on the shape of the $n_i(\tau)$ distribution (which varies
with field rotation) in a way the window controller does not.
...
We accept that framing; the taxonomy offers a compact, mechanistically
grounded description of the observed diversity of outcomes.
...
controllers with a fixed-direction prior, but quantifying
$c(\alpha/\sigma, M)$ requires a parameter sweep beyond this
paper's scope.
...
$\bar{K}{\approx}8$, $dt{=}0.02$); back-of-envelope arguments for
the scaling are given in Section~\ref{sec:discussion} but require
a systematic parameter sweep to validate.
...
a clean crossover at $\Lmax/\Tcorr \approx 0.30$; empirical threshold
earlier than the $L^* \propto (\omdt)^{-2/3}$ bias/noise prediction
because the capacity-matching policy collapses coordination before
directional parity is reached.
\emph{Sinc-attenuation regime} ($\omega{\in}\{2.0,\,2.5,\,3.0\}$):
amplitude damping scales down both the noise-reduction benefit and the
stale-direction cost together, suppressing the sharp crossover.
\emph{Rapid-rotation regime} ($\omega{\in}\{10.0,\,17.0\}$):
$\Lmax$ falls below integer resolution; mechanism present but
undetectable at $n{=}32$.
