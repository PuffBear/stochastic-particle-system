...
beneficial.  At slow rotation ($\omega{=}1.5$), instrumented runs show the
mechanism's predicted directional signatures: an observed minimum
in shared-arm angular error at $\Lmax$, and inter-agent angular
dispersion structurally zero in the shared arm and non-zero in the
independent arm.  The coordination benefit landscape under rotation is
...
sinc-attenuation regime ($\omega{\in}\{2.0,\,2.5,\,3.0\}$) dampens
...
\begin{enumerate}
  ...
  failure mechanism: instrumented runs simultaneously show a U-shaped
  shared-arm angular error (minimum at exactly $\Lmax$) and inter-agent
  directional dispersion collapsing to zero in the shared arm while the
  independent arm retains diversity (Figure~\ref{fig:dispersion}).
  These two directional signatures are consistent with the proposed
  mechanism.  We do not measure spatial trajectory overlap, spatial
  coverage directly, or provide a formal mediation analysis connecting
  angular dispersion to captures; those remain open.
  Directional diagnostics are performed only at slow ($\omega{=}1.5$);
  the performance pattern at mid ($\omega{=}5.0$) is consistent with
  the mechanism, but the directional signatures were not measured there.
...
\begin{equation}
  a_m = \mathrm{unit}\!\bigl(\beta_t\,\hat{f} + (1-\beta_t)\,\hat{d}_m\bigr),
  \qquad
...
position of visible particles, and the blend weight
$\beta_t = \min(0.7,\,2\,\bar{n}_t/N_{\max})$ rises with the current
mean per-agent particle count $\bar{n}_t$.
...
nonlinearity arises here: a high blend weight $\beta_t$ causes all
...
\noindent\textbf{Terminology.}  Three distinct $L$-based thresholds appear
in this section; they coincide only under specific conditions:
\begin{itemize}[noitemsep,topsep=2pt]
  \item \emph{Significance threshold} ($\Lmax$): the largest $L$ with
        $p{<}0.05$ in the pre-registered one-sided $t$-test.  This is
        a statistical-detectability threshold, not a behavioural reversal.
  \item \emph{Zero-crossing threshold} ($L^*$): the value of $L$ at which
        $\mathbb{E}[\Delta]$ changes sign, estimated by interpolation between
        adjacent grid points.
  \item \emph{Coordination-collapse boundary}: the $L$ at which the expected
        number of captures under the shared arm falls below the independent
        arm — equivalent to the zero-crossing.  We use this phrase only when
        the evidence supports a genuine sign reversal, i.e.\ when
        $\Lmax \approx L^*$.
\end{itemize}
For the slow condition ($\omega{=}1.5$), $\Lmax{=}10$ and
$L^*{\in}(10,20)$ are in the same interval:
$\Lmax$ is an approximate coordination-collapse boundary.
...
episodes at $\omega{=}1.5$ (slow, $n{=}32$ seeds) recording at
every step~$t$: the true field direction $\theta(t)=\theta_0+\omega
t\,dt$; the shared arm's direction estimate $\hat\theta_{\rm shared}$
(windowed team mean, identical for all agents by construction); and
each agent's independent estimate $\hat\theta_{\rm indep}^{(m)}$
(own windowed mean).

Figure~\ref{fig:dispersion} reveals two simultaneous signatures:

\begin{enumerate}
  \item \textbf{Shared arm error is U-shaped with observed minimum
  at $\Lmax$.}
  Mean angular error $|\hat\theta_{\rm shared}-\theta_{\rm true}|$
  falls from 0.768~rad at $L{=}1$ to 0.286~rad at $L{=}10$
  (noise reduction), then rises to 0.575~rad at $L{=}67$ as
  angular lag $\omega(L{-}1)dt/2$ accumulates.  The observed
  minimum lies at $L{=}10$; adjacent comparisons
  ($E_{\rm shared}(5){>}E_{\rm shared}(10)$: $t{=}3.76$, and
  $E_{\rm shared}(20){>}E_{\rm shared}(10)$: $t{=}1.89$, both
  unpaired, df${\approx}61$) support the minimum but a
  paired test awaits per-seed archival.  The independent arm's
  error decreases from 0.566 to 0.488~rad between $L{=}10$ and
  $L{=}20$ and has no such minimum, as noise — not lag —
  drives its error at short to moderate $L$.

  \item \textbf{Shared arm inter-agent dispersion collapses to zero.}
  Because all agents receive the identical team estimate,
  directional spread across agents is zero for every $L$.
  Independent agents maintain 0.585~rad of inter-agent diversity
  at $L{=}10$ and 0.343~rad even at $L{=}67$.
\end{enumerate}

The directional ingredients predicted by the mechanism are directly
measured; their causal mediation of capture performance is not
isolated.  In particular, shared-arm inter-agent dispersion is zero
by construction at \emph{every} $L$ — including $L{=}1$ where
coordination is beneficial.  The zero-dispersion is a structural
property of shared estimation, not a collapse that occurs at $\Lmax$.
What changes at $\Lmax$ is the interaction: zero inter-agent diversity
compounded by a stale field direction, rather than zero diversity
compounded by an accurate one.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_dispersion}
\Description{Two-panel figure. Panel (a): mean angular error in radians versus L for shared arm (blue circles) and independent arm (red squares), with shaded standard-error bands. Shared arm is U-shaped with minimum at L=10; independent arm decreases monotonically. Dotted line shows predicted angular lag omega*(L-1)*dt/2. Panel (b): inter-agent circular standard deviation versus L. Shared arm is flat at zero; independent arm decreases from 1.165 rad at L=1 to 0.343 rad at L=67.}
\caption{Directional diagnostics, $\omega{=}1.5$ (slow, $n{=}32$
seeds, 9001--9032).
Shaded bands are $\pm$1~SE across seeds.
\textbf{(a)} Mean angular error $|\hat\theta-\theta_{\rm true}|$ vs.\
$L$.  Shared arm (blue) shows an observed minimum at $L{=}10$
(0.286~rad), then rises as predicted angular lag $\omega(L{-}1)dt/2$
(dotted) accumulates.  Adjacent-comparison tests support the minimum
($E_{\rm shared}(5){>}E_{\rm shared}(10)$: $t{=}3.76$;
$E_{\rm shared}(20){>}E_{\rm shared}(10)$: $t{=}1.89$; both unpaired).
Independent arm (red) decreases monotonically.
\textbf{(b)} Inter-agent directional dispersion (circular std across
agents).  Shared arm is identically zero for all $L$ by construction;
independent arm retains 0.585~rad at $L{=}10$, 0.343~rad at $L{=}67$.
Zero shared-arm dispersion is structural, not a collapse at $\Lmax$;
the harmful regime is zero diversity \emph{combined with} a stale direction.}
...
(vii)~The mechanism isolation (Figure~\ref{fig:dispersion}) directly
measures inter-agent angular dispersion and shared-arm angular bias.
It does not measure spatial trajectory overlap, spatial coverage, or
provide a formal mediation chain from angular dispersion to capture
counts; that causal chain is consistent with the evidence but not
directly tested.
