...
independently at two different rotation speeds. A pre-experiment analytical
...
Intermediate $L$ values were concentrated at the two interior
rotation speeds (slow, mid) by design: very\_slow has
$\Lmax{=}67$ (boundary, unresolvable within the grid) and fast
has predicted $\Lmax{<}1$ step (below grid resolution); additional
$L$ cells at those conditions would not sharpen the $\Lmax$ estimate.
...
The force-through-origin OLS fit on the three rotation speeds with
defined $\Lmax$ (very\_slow, slow, mid) gives $\Lmax \approx 0.85
\cdot \Tcorr$ ($R^2{=}0.80$), inflated by the boundary-censored
very\_slow point.  \textbf{Restricting to the two interior anchor
points (slow, mid) yields $c = 0.30$ exactly.}  The headline
constant $c \approx 0.30$ therefore rests on two anchor points; we
report it as a precise preliminary value requiring validation at
additional rotation speeds, not as an asymptotically established
constant.
...
Figure~\ref{fig:scaling} plots $\Lmax$ versus $\Tcorr$ for
all rotation speeds with defined $\Lmax$.  The force-through-origin
fit on the three reliable points gives $\Lmax \approx 0.31\,\Tcorr$.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_scaling}
\Description{Scatter plot of L-max versus T-corr with two fitted lines (pre-experiment c=1.0 dashed, empirical c=0.85 solid) and three data points: very-slow (open square at T=67), slow (filled circle at T=33, L=10), mid (filled circle at T=10, L=3).}
\caption{$\Lmax$ (last significant $L$, window method) vs.\
$\Tcorr = 1/(\omdt)$.  Filled circles: two clean interior points
(slow, mid); square: very\_slow (boundary — any $\Lmax \in [45,67]$
maps to 67 in our $L$ grid).  Dashed: pre-experiment prediction
($c{=}1.0$); solid: force-through-origin fit on three points
including the boundary very\_slow ($c{=}0.85$, inflated by boundary
censoring); the two interior anchors alone give $c{=}0.30$ exactly.
