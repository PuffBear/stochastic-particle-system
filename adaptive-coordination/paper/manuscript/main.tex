...
At two interior rotation speeds, the pre-registered
significance-based threshold satisfies
\[
  \Lmax(\omega) / \Tcorr \;\approx\; 0.30,
  \quad \Tcorr = 1/(\omdt),
\]
where $\Tcorr$ is the field's one-radian decorrelation timescale.
An additional intermediate condition and multiplicity analysis
show that this pattern is preliminary.
...
We demonstrated a stale-coordination failure mechanism in which
the shared arm drives all collectors confidently toward a
systematically stale direction, eliminating the coverage diversity
that makes coordination beneficial.
In the clearest slow-rotation condition, this failure appears after
the memory window spans approximately $0.30\;\text{rad}$ of total
field rotation; the significance-based threshold takes the same
value at one additional interior speed.
The largest benefit window at these two speeds satisfies
$\Lmax \approx 0.30 \cdot \Tcorr$, with a boundary-censored
very\_slow point consistent but not conclusive.  The theoretical
