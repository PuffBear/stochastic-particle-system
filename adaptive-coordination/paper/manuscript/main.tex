...
% ── AAMAS metadata ───────────────────────────────────────────
\title{When Shared Memory Hurts: Stale-Coordination Failure and Memory-Window\\
       Scaling in Multi-Agent Particle Collection under a Rotating Latent Field}
...
fully matched experiment across four primary rotation speeds
($\Tcorr \in \{67, 33, 10, 3\}$ steps) plus a supplementary
third-anchor condition ($\Tcorr{=}20$), and eight memory lengths
...
\[
  ...
  \frac{0.30}{\omdt},
\]
where $\Tcorr = 1/(\omdt)$ is the field's one-radian decorrelation
timescale in steps (the time for the field to rotate by one radian).
At crossover the memory window spans approximately
$17^\circ$ ($0.30\;\text{rad}$) of total field rotation — verified
at two interior rotation speeds (slow, mid); a supplementary
third condition shows complex behaviour discussed in the paper.
A pre-experiment analytical
...
controller does not follow the same scaling. For this environment
and controller, a sliding window of $L \lesssim 0.30/(\omdt)$
preserves the coordination benefit when the latent field drifts
at per-step rate $\omdt$; the constant $c \approx 0.30$ is
specific to the studied parameter regime and requires re-validation
under different agent counts or field strengths.
\end{abstract}
...
\[
  ...
  \Tcorr = \frac{1}{\omdt},
\]
where $\Tcorr$ is the one-radian decorrelation timescale of the
field (the time to rotate by one radian).
At crossover, the total field rotation over the window is
$\omega \cdot \Lmax \cdot dt \approx 0.30\;\mathrm{rad}$,
a pattern observed at two interior rotation speeds
($\Tcorr \in \{33, 10\}$) under the pre-registered per-cell criterion.
The constant $c \approx 0.30$ is preliminary; the borderline
slow anchor ($p{=}0.049$) motivates further validation.
...
\begin{enumerate}
  \item We demonstrate $\Lmax \propto \Tcorr$ at two interior
  rotation speeds (slow, mid; $\Tcorr \in \{33, 10\}$), with
  the proportionality constant $c \approx 0.30$ consistent
  across these two anchors (plus a boundary very\_slow point at
  $\Tcorr{=}67$). A supplementary $\Tcorr{=}20$ condition shows
  anomalous breadth without a sharp crossover and is discussed
  separately; the headline $c \approx 0.30$ is a preliminary
  two-anchor result, not a settled universal constant.
...
\caption{Window-method per-cell results.
         ...
         stationary cell coincidentally yields the same value because
         field stationarity makes the window length irrelevant.
...
The significance-based $\Lmax$ and the zero-crossing $L^*$
bracket the true crossover from different directions; we use $\Lmax$
throughout as the pre-registered quantity.

\begin{table}[b]
\centering
\caption{$\Lmax$ (last $L$ with $p{<}0.05$, window) and
         ratio to $\Tcorr$.  The two clean interior points
         (slow, mid) both give $\Lmax/\Tcorr = 0.30$.
         $\dagger\omega{=}2.5$: no reliable $\Lmax$ — positive
         benefit persists across the full $L$ grid without a
         sharp crossover; see Sec.~\ref{sec:lmax-scaling}.}
\label{tab:lmax}
\setlength\tabcolsep{5pt}
\begin{tabular}{lrrrr}
\toprule
...
holds at an intermediate rotation speed.  The result is anomalous:
positive mean $\Dbar$ at every $L$ in the grid (Table~\ref{tab:main}),
with sign-majority (${>}n/2$ seeds benefiting) maintained through
$L{=}30$ and the $t$-test significant through $L{=}45$.  Under the
same sequential stopping rule used for slow and mid — last $L$ where
sign count ${>}n/2$ before the first failure — this gives $\Lmax{=}30$
($c{=}30/20{=}1.50$), well above the $c{=}0.30$ prediction.  The
$\omega{=}2.5$ condition therefore \emph{cannot serve as a clean third
anchor}: the coordination benefit is too broad and flat to resolve a
sharp $\Lmax$ within our grid.  \textbf{The headline $c \approx 0.30$
...
\[
  \omega \cdot \Lmax \cdot dt = 0.30\;\text{rad}.
\]
The memory window that preserves coordination benefit spans
approximately $17^\circ$ of total field rotation.  The
$\omega{=}2.5$ condition does not yield a reliable $\Lmax$
estimate and is excluded from this calculation.

Figure~\ref{fig:scaling} plots $\Lmax$ versus $\Tcorr$ for the
two clean interior anchors and the boundary very\_slow point
($\omega{=}2.5$ is shown but excluded from the fit; see caption).
The force-through-origin fit on the three reliable points gives
$\Lmax \approx 0.85\,\Tcorr$ ($R^2{=}0.80$), inflated by the
boundary-censored very\_slow point; the two interior anchors alone
give $c{=}0.30$ exactly.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_scaling}
\Description{Scatter plot of L-max versus T-corr with two fitted lines (pre-experiment c=1.0 dashed, empirical c=0.85 solid) and four data points: very-slow (open square at T=67, L=67), slow (filled circle at T=33, L=10), mid (filled circle at T=10, L=3), and omega=2.5 (open triangle at T=20, L-max ambiguous, shown for reference but excluded from the fit).}
\caption{$\Lmax$ (last significant $L$, window method) vs.\
$\Tcorr = 1/(\omdt)$.  Filled circles: two clean interior anchors
(slow, mid); square: very\_slow (boundary); triangle: $\omega{=}2.5$
(shown for reference — no reliable $\Lmax$, excluded from fit).
Dashed: pre-experiment prediction ($c{=}1.0$); solid:
force-through-origin fit on three reliable points including
boundary very\_slow ($c{=}0.85$, inflated by boundary censoring);
...
$c \approx 0.30$ claim; this sensitivity should be viewed as
motivation for validation at additional rotation speeds with
non-borderline $\Lmax$ values rather than adding seeds post-hoc.
...
(vi)~The two-anchor result ($\Tcorr \in \{33,10\}$) is a
preliminary pattern, not a law: the borderline slow anchor
($p{=}0.049$) does not survive uniform Holm correction, and the
supplementary $\omega{=}2.5$ condition does not conform to the
$c{=}0.30$ prediction (Section~\ref{sec:anchor-robustness}).
Additional, non-borderline anchors are needed before the
scaling constant can be treated as reliable (see Anchor robustness paragraph above).
...
We demonstrated a stale-coordination failure mechanism: once the
sliding-window memory spans more than $\approx 0.30\;\text{rad}$
of total field rotation, the shared arm drives all collectors
confidently toward a systematically stale direction, eliminating
the coverage diversity that makes coordination beneficial.  The
largest memory window providing a positive benefit scales as
$\Lmax \approx 0.30 \cdot \Tcorr = 0.30/(\omdt)$ at two interior
rotation speeds ($\Tcorr \in \{33, 10\}$), with a boundary
consistent but censored very\_slow point.  The theoretical
prediction ($c \approx 1.0$) overshot by a factor of three because
the relevant threshold is not directional MSE parity but the point
at which shared-arm confidence in a stale direction outweighs the
coverage diversity of independent agents — a policy nonlinearity
inaccessible to information-geometric arguments.

Within the specific parameter regime studied ($\alpha{=}0.06$,
$\sigma{=}0.06$, $M{=}4$, $dt{=}0.02$, sliding-window controller),
the heuristic $L \lesssim 0.30/(\omdt)$ avoids this failure mode.
The constant $c \approx 0.30$ is a preliminary two-anchor result;
the borderline slow anchor ($p{=}0.049$) and the anomalous
$\omega{=}2.5$ condition both motivate further validation before
this constant is used prescriptively.  The exponential-decay
controller does not follow the same scaling, making the sliding
window the more predictable choice for memory-window selection
under known $\omega$ in this setting.
...
$\alpha\,\mathrm{sinc}(\omega L\,dt/2)$.
...
so sinc attenuation is negligible \emph{in this region}.
Beyond $\Lmax$, the approximation breaks down: at
$(\omega{=}5, L{=}67)$ the argument is $5{\times}67{\times}0.02/2 = 3.35\;\text{rad}$,
and $\mathrm{sinc}(3.35) = \sin(3.35)/3.35 \approx -0.06$, which
is not negligible.  However, these cells are already in the
degraded regime where $\Dbar < 0$; the mechanism there is angular
bias, not sinc attenuation.
The dominant factor throughout is the angular lag $\angbias = \omega L\,dt/2$.
