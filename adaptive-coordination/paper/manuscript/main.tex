...
runs show the mechanism's predicted directional signatures: an observed
minimum in shared-arm angular error at $\Lmax$ (adjacent-comparison
$t{=}3.76$ at both speeds), and inter-agent angular dispersion
...
\begin{enumerate}
  ...
  Directional diagnostics are performed at both interior speeds
  (slow $\omega{=}1.5$ and mid $\omega{=}5.0$); both show an observed
  minimum in shared-arm angular error at $\Lmax$ and non-zero
  independent-arm dispersion throughout (Figure~\ref{fig:dispersion}).
...
At mid we used $L{\in}\{3,10,30,45\}$; at slow, all eight $L$ levels.
...
\begin{enumerate}
  ...
  at $\Lmax$.}
  \emph{Slow:} mean angular error falls from 0.768~rad at $L{=}1$
  to 0.286~rad at $L{=}10$, then rises to 0.575~rad at $L{=}67$.
  Adjacent comparisons support the minimum ($E(5){>}E(10)$: $t{=}3.76$;
  $E(20){>}E(10)$: $t{=}1.89$; both unpaired, df${\approx}61$).
  \emph{Mid:} error falls from 0.471~rad at $L{=}3$ ($\Lmax$) to a
  steep rise — 0.538 at $L{=}10$, 1.215 at $L{=}30$ — as sinc
  attenuation dominates beyond the linear-rotation regime
  ($E(3){<}E(10)$: $t{=}3.76$, unpaired, df${\approx}61$).
  In both conditions the observed minimum aligns with $\Lmax$.
...
Shared arm (blue) shows an observed minimum at $\Lmax$ in both conditions
($L{=}10$ at slow, $L{=}3$ at mid), confirmed by adjacent-comparison
$t$-tests ($t{=}3.76$ in each case, unpaired).  Beyond $\Lmax$ the shared
arm rises due to angular lag (slow, linear-rotation regime) or sinc
