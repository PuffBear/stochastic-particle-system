...
\begin{enumerate}
  ...
  failure mechanism: instrumented runs simultaneously show U-shaped
  shared-arm angular error with an observed minimum at $\Lmax$ and
  inter-agent directional dispersion collapsing to zero in the shared arm
  while the independent arm retains diversity
  (Figure~\ref{fig:dispersion}).
  Both interior speeds exhibit U-shaped shared-arm angular error profiles
  with observed minima at their respective $\Lmax$; the independent arm
  shows no corresponding minimum at either speed.
