...
The primary finding is a \emph{stale-coordination failure
mechanism}: once the sliding-window memory spans sufficient
rotation, the shared arm drives all collectors confidently toward
a systematically wrong direction, eliminating the coverage
diversity that makes coordination beneficial.
The significance-based threshold is consistent with
$\Lmax \approx c\,\Tcorr$ ($\Tcorr = 1/(\omdt)$, the
one-radian decorrelation timescale), where $c$ ranges from
$0.15$ (Holm-corrected slow anchor) to $0.30$ (uncorrected) across
the two clean interior rotation speeds tested; a third rotation
speed ($\Tcorr{=}20$) did not reproduce a sharp crossover at all.
The constant $c$ should be treated as a preliminary point estimate,
not a settled scaling law. The exponential-decay controller does
...
(Holm-corrected) to $0.30$ (uncorrected) across the two interior
speeds; a boundary-censored very\_slow point is consistent but
not conclusive; and a third tested speed did not reproduce a sharp
crossover.  The constant $c$ is a preliminary point estimate, not
a settled scaling law.  The bias/noise-parity theory predicts $L^* \propto (\omega\,dt)^{-2/3}$,
