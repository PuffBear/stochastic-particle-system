...
A pre-experiment bias/noise-parity analysis predicts
$L^* \propto (\omega\,dt)^{-2/3}$, a different functional form
from the observed $\Lmax \propto (\omega\,dt)^{-1}$;
the mechanism for the steeper empirical scaling is a policy
nonlinearity in which the shared arm confidently drives all
collectors toward a systematically stale direction, crowding out
the coverage diversity that makes independent coordination
beneficial. The exponential-decay
...
\begin{enumerate}
  \item We show that $\Lmax/\Tcorr \approx 0.30$ at two interior
  rotation speeds (slow, mid; $\Tcorr \in \{33, 10\}$), consistent
  with a boundary very\_slow point ($\Tcorr{=}67$). A supplementary
  $\Tcorr{=}20$ condition does not show the same ratio and is
  discussed separately; the pattern is a preliminary two-anchor
  observation, not a settled proportionality.
  \item We show that the empirical $\Lmax \propto (\omega\,dt)^{-1}$
  pattern is steeper than the bias/noise-parity prediction
  $L^* \propto (\omega\,dt)^{-2/3}$, and that the crossover
  occurs at lower $L$ than the theory predicts, attributable to
  policy nonlinearity rather than information geometry.
...
Dashed: $L{=}\Tcorr$ reference line (not a theoretical prediction — see text); solid:
...
very\_slow point consistent but not conclusive.  The bias/noise-parity theory predicts $L^* \propto (\omega\,dt)^{-2/3}$,
