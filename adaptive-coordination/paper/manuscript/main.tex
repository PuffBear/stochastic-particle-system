...
structurally zero in the shared arm and non-zero in the independent arm.  The coordination benefit landscape under rotation is \emph{regime-dependent}: qualitatively different outcomes arise at
...
Communication protocols designed for a stationary environment fail
when the field drifts: a team converging on a shared but stale estimate
may perform \emph{worse} than independent agents acting on their own
local observations.
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \emph{Coordination-collapse boundary}: the $L$ at which the
        ...
        this phrase only when $\Lmax \approx L^*$.
\end{itemize}
...
The theoretical prediction ($\Lmax \approx 0.30 \times 3 = 0.9$ steps)
falls below grid resolution; the null result is expected and provides
no independent evidence against the scaling law.
...
\caption{Window-method per-cell results.
         ...
         Dashes: $L$ not run for that $\omega$.
         $\dagger$Full-history ($L{=}H$) stationary cell ($n{=}8$);
         field stationarity removes temporal bias, making this
         a comparison baseline, not the reproduction gate.
         $\omega{=}2.5$: supplementary (Sec.~\ref{sec:lmax-scaling}).
         Exploratory $\omega{\in}\{1.0,\,2.0,\,3.0,\,10.0\}$ excluded;
         $\Lmax$ in Table~\ref{tab:lmax}.}
\label{tab:main}
\setlength\tabcolsep{5pt}
...
Figure~\ref{fig:crossover} shows $\Dbar$ versus $L$ for slow and mid
with 95\% confidence intervals.
At slow, $\Lmax{=}10$ and $L^*{\in}(10, 20)$ co-locate the crossover
in the same $L$-interval, confirming slow as a genuine
coordination-collapse boundary (Section~\ref{sec:threshold-defs}).
At mid the sharp-crossover character is absent: $\Dbar$ shrinks
gradually rather than collapsing, and post-hoc power for the $L{=}5$
cell ($\Dbar{=}{+}0.22$, $d{\approx}0.09$) is below 15\% — consistent
with the detectability interpretation of $\Lmax{=}3$ established in
...
\begin{enumerate}
  ...
  paired-bootstrap 95\% CI $[{+}0.041,{+}0.090]$~rad.
...
performance is not isolated.  What changes at $\Lmax$ is not a
collapse of shared-arm dispersion — that is structural at every $L$
— but the interaction: zero diversity compounded by a stale field
direction rather than an accurate one.
...
paired-bootstrap 95\% CIs exclude zero).
\emph{Mid ($\Lmax{=}3$):} error decreases from 0.770~rad at $L{=}1$ to
0.471~rad at $\Lmax{=}3$ (paired $t(31){=}34.40$, $p{<}0.001$), then
...
multi-agent sharing amplifies directional bias, making the constant
smaller than the single-agent SW-UCB optimum.
...
Under uniform Holm correction across slow's eight cells, the adjusted
threshold at $L{=}10$ is $0.05/5{=}0.01$; the borderline $p{=}0.049$
fails it, shifting $\Lmax(\text{slow})$ to 5 steps ($c{\approx}0.15$).
The mid anchor ($p{=}0.004$) survives.
The $c{\approx}0.30$ claim rests on the pre-registered per-cell
$p{<}0.05$ criterion (uncorrected); post-hoc power at $n{=}32$ is
${\approx}50\%$, below the 80\% design target.
A pre-specified replication at $n{=}64$ confirms the slow anchor
($\Dbar{=}{+}0.70$, $t{=}{+}1.87$, $p{<}0.05$) and preserves
the crossover at $L{=}20$ ($\Dbar{=}{+}0.14$, NS).

\paragraph{The regime taxonomy: post-hoc structure with mechanistic accounts.}
The taxonomy was constructed post-hoc; we accept the reading that it is
an explanation built around counterexamples.
Each non-conforming regime has a characterisable mechanistic account.
\emph{Sinc-attenuation} ($\omega{\in}\{2.0, 2.5, 3.0\}$): amplitude
damping suppresses the crossover by scaling down both benefit and cost
proportionally; boundary at $\omega{\in}(3.0, 5.0)$.
\emph{Rapid-rotation} ($\omega{=}10.0$): $\Lmax$ falls below integer
resolution; the mechanism is verified present but undetectable at $n{=}32$.
\emph{Long-episode} ($\omega{=}1.0$): unresolved.
Diagnosis: $\omega L\,dt \lesssim 0.30\;\text{rad}$ indicates the
linear-rotation regime ($c{\approx}0.30$ applies); substantially larger
values indicate sinc-attenuation; $c\,\Tcorr{<}1$ indicates
rapid-rotation.  The central open question is what determines which
regime a given $(\omega, H)$ pair falls into.

\paragraph{Generality and functional dependence of $c$.}
$c \approx 0.30$ is specific to the SPS-C03 parameter point
($\alpha{=}0.06$, $\sigma{=}0.06$, $M{=}4$).
Back-of-envelope analysis ($\sigma_{\theta}\propto\sigma/(\alpha\sqrt{M\bar{K}L\,dt})$)
suggests $c$ is increasing in $\alpha/\sigma$ and weakly in $M$:
larger or stronger teams tolerate more staleness.
We hypothesise $\Lmax \propto \Tcorr$ extends to other fixed-direction
memory controllers, but quantifying $c(\alpha/\sigma, M)$ requires a
parameter sweep.

An exploratory $\omega{\times}L$ sweep ($n{=}16$, shared arm only)
finds $\omdt{\cdot}\Lopt{\in}\{0.30,\,0.21,\,0.30\}$ for the three
linear-regime conditions, consistent with $\hat\Gamma{=}0.29$;
bootstrap CIs are wide and the result is suggestive but not confirmatory
(Figure~\ref{fig:gamma_sweep}, Appendix~\ref{app:gamma}).

\paragraph{Limitations.}
(i--ii)~Field rotation is deterministic; the controller is scripted.
(iii--iv)~One coordination mechanism tested (count-weighted mean);
$n{=}32$ gives ${\approx}80\%$ power for $|\Dbar|{\approx}0.9$.
(v)~$c{\approx}0.30$ is at a single parameter point within the
linear-rotation regime; the slow condition shows genuine sign reversal
($\Lmax{\approx}L^*$); mid's $\Lmax{=}3$ marks detectability.
(vii--viii)~Directional diagnostics measure angular bias and dispersion
but not a formal mediation chain; the taxonomy is data-derived and the
regime-membership rule is unestablished.
...
Four regimes emerge.
\emph{Linear-rotation} ($\omega\Lmax dt{\approx}0.30\;\text{rad}$):
clean crossover at $\Lmax/\Tcorr{\approx}0.30$; empirical threshold
earlier than the $L^*{\propto}(\omdt)^{-2/3}$ prediction because
capacity-matching collapses coordination before directional parity.
\emph{Sinc-attenuation} ($\omega{\in}\{2.0,\,2.5,\,3.0\}$):
amplitude damping suppresses the crossover.
\emph{Rapid-rotation} ($\omega{\in}\{10.0,\,17.0\}$):
$\Lmax$ below integer resolution; mechanism undetectable at $n{=}32$.
\emph{Long-episode} ($\omega{=}1.0$, $\Tcorr{\approx}H$):
only $L{=}1$ significant; unresolved.
...
Code, data, and pre-registration materials are available at \url{[anonymized]}.
...
\section{Reward-Optimal Window Sweep}
\label{app:gamma}

\begin{figure*}[h]
\centering
\includegraphics[width=\textwidth]{fig_gamma_sweep}
...
exploratory). $\Lopt(\omega) = \arg\max_L\,\mathbb{E}[Y_{\mathrm{shared}}(L)]$,
distinct from $\Lmax$ and $L^*$.
\textbf{Left:} Shared-arm reward vs.\ $L$; linear-regime curves (solid)
peak at moderate $L$; anomalous conditions (dashed) are flat or
boundary-limited.
\textbf{Centre:} $\Lopt$ vs.\ $\Tcorr$; force-through-origin fit,
slope $0.29$ (linear-regime points only).
\textbf{Right:} $\Gamma = \omdt\cdot\Lopt$ vs.\ $\omega$ with bootstrap
95\% CIs ($B{=}4000$); CIs are wide and the $\omega{=}1.5$ point falls
below the ${\pm}20\%$ band --- suggestive but not confirmatory.}
