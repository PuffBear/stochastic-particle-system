...
The central result is a \emph{stale-coordination failure
...
eliminating the coverage diversity that makes coordination
beneficial.  The significance-based crossover is consistent with
$\Lmax \approx c\,\Tcorr$ (where $\Tcorr = 1/(\omdt)$ is the
field's one-radian decorrelation timescale) with $c$ ranging from
$0.15$ (Holm-corrected) to $0.30$ (uncorrected) at two interior
rotation speeds; a third tested speed did not reproduce a sharp
crossover, and $c$ should be read as a preliminary point estimate.
The exponential-decay controller does not show the same pattern.
\end{abstract}
...
At these two speeds the window spans $\approx 0.30\;\mathrm{rad}$
of total field rotation at the threshold.
The pattern is preliminary (see Section~\ref{sec:lmax-scaling}).
...
\begin{enumerate}
  \item We demonstrate a stale-coordination failure mechanism:
  confidence in a shared but stale direction eliminates the
  coverage diversity of independent agents, reversing the
  coordination benefit. This is well-supported in the slow
  rotation condition and observed, with less sharpness, in a
  second interior speed.
  \item We report a preliminary point estimate: the
  significance-based threshold satisfies $\Lmax/\Tcorr \approx 0.30$
  (uncorrected) to $0.15$ (Holm-corrected) at two interior speeds,
  with a third tested speed not reproducing a sharp crossover.
  This is an exploratory pattern, not a settled scaling law.
...
\caption{$\Lmax$ (last $L$ with $p{<}0.05$, window) and
         ratio to $\Tcorr$.  Bold: two clean interior points
         (slow, mid) with $\Lmax/\Tcorr = 0.30$.
         $\P\;\omega{=}1.0$: only $L{=}1$ significant ($t{=}2.68$);
         all $L{\geq}3$ non-significant ($t{<}0.5$).
         $\S\;\omega{=}2.5$: $\Lmax{=}45$ by $t$-test criterion ($t{=}1.86$),
         but $t$-profile non-monotone; see Sec.~\ref{sec:lmax-scaling}.
         $\ddagger\;\omega{=}2.0$: all $L$ in grid significant (min.\ $t{=}1.93$);
         $\Lmax$ is a lower bound (${\geq}67$).}
\label{tab:lmax}
\setlength\tabcolsep{5pt}
\begin{tabular}{lrrrr}
\toprule
...
1.0  & 50 & 1$^{\P}$     & 0.02 rad          & 0.02 \\
1.5  & 33 & 10            & \textbf{0.30 rad} & \textbf{0.30} \\
2.0  & 25 & ${\geq}$67$^{\ddagger}$ & ${\geq}$2.68 rad & ${\geq}$2.68 \\
2.5  & 20 & 45$^{\S}$    & 2.25 rad          & 2.25 \\
...
\medskip
\noindent\textbf{Episode-rotation regime and sinc attenuation.}
...
For $\omega{\in}\{2.0, 2.5\}$ the picture is different.
At $L{=}45$ the sinc factor is $0.87$ ($\omega{=}2.0$)
and $0.80$ ($\omega{=}2.5$); at $L{=}67$ it falls to
...
collapsing sharply.
We hypothesise that this sinc-regime effect suppresses the sharp
crossover, explaining the anomalously high $\Lmax$ for these conditions.
The episode total-rotation figures — $\omega H\,dt = 2.68$ and
$3.35\;\text{rad}$ — place both conditions in an intermediate regime
where sinc attenuation is substantial across the entire tested $L$ range.
...
The anomalous conditions are those where sinc attenuation pervades
the tested $L$ range or where episode-level variance is inflated.
...
$\omega{=}2.0$ and $\omega{=}2.5$ conditions do not yield
...
$\Lmax$ is only $0.38$ (slow) and $0.21$ (mid) — the empirical
crossover is earlier than the theory predicts.
...
Concretely: at $n{=}32$ and the observed effect ($\Dbar{=}{+}0.94$,
SD~3.20, $d{\approx}0.29$), post-hoc power for the one-sided
$t$-test is $\approx 50\%$ — below the 80\% design target, which
assumed $|\Dbar| \geq 0.90$ with SD~2.5.  The one-sided 95\% lower
confidence bound is $\Dbar - t_{0.05,31}\cdot\mathrm{SE}
\approx 0.94 - 1.70{\times}0.57 \approx {-}0.03$ particles:
consistent with zero.  This does not invalidate the pre-registered
result, but it means the anchor identifies a \emph{direction} of
effect at this $L$, not a precise effect magnitude.

\paragraph{The anomaly cluster as a second open problem.}
The three non-conforming conditions ($\omega{\in}\{1.0, 2.0, 2.5\}$) are not
random scatter: their $c$ values span from $0.02$ (anomalously low) to
$\geq 2.68$ (anomalously high), flanking the two clean anchors at $c{=}0.30$.
Section~\ref{sec:lmax-scaling} proposes a sinc-regime account for
$\omega{\in}\{2.0, 2.5\}$: these conditions have significant amplitude
attenuation ($\mathrm{sinc} \in [0.59, 0.87]$) across all tested $L$,
which weakens the ``confident-wrong-direction'' mechanism and suppresses
the sharp crossover.
If the total episode rotation $\omega H\,dt$ is the relevant regime variable,
then the clean $\Lmax \propto \Tcorr$ scaling may hold only within a
specific range of $\omega H\,dt$, and the apparent universality of
$c \approx 0.30$ across two interior speeds may reflect that both happen
to lie in this range.  This reframes the anomalies from a weakness of the
two-anchor result into the paper's central open question:
...
($p{=}0.049$) does not survive uniform Holm correction, and three
exploratory speeds do not conform to $c{=}0.30$
(Section~\ref{sec:lmax-scaling}).
A sinc-regime hypothesis for $\omega{\in}\{2.0, 2.5\}$ is developed
in Sec.~\ref{sec:lmax-scaling}: amplitude attenuation
($\mathrm{sinc} \in [0.59, 0.87]$ at large $L$) may suppress the
sharp crossover, making these conditions structurally different from
the clean interior anchors rather than contradicting them.
The $\omega{=}1.0$ anomaly is not explained by sinc attenuation and
may reflect a power limitation ($n{=}32$ is underpowered for the
observed effect size at $L{\geq}3$) or a different episode-level
interaction.  Whether $c{=}0.30$ holds at additional clean interior
speeds is the central open question (see also the anomaly-cluster
discussion in Sec.~\ref{sec:discussion}).
...
The significance-based threshold is consistent with
$\Lmax \approx c\,\Tcorr$, where $c$ ranges from $0.15$
(Holm-corrected) to $0.30$ (uncorrected) at the two clean interior
speeds ($\omega{\in}\{1.5,\,5.0\}$); a boundary-censored
very\_slow point is consistent but not conclusive.
Exploratory runs at three further speeds do not conform.
For $\omega{\in}\{2.0, 2.5\}$ (anomalously high $\Lmax$), a sinc-regime
hypothesis offers a partial account: amplitude attenuation
($\mathrm{sinc}{\in}[0.59, 0.87]$ at large $L$) weakens the
``confident-wrong-direction'' mechanism throughout the tested $L$ grid,
suppressing the sharp crossover.
For $\omega{=}1.0$ ($\Lmax{=}1$, anomalously low),
the cause is not resolved but may involve power
limitations or episode-level variance inflation.
The constant $c$ is a preliminary two-speed observation;
the anomaly cluster raises the question of what determines whether
a given $(\omega, H)$ pair produces a sharp crossover at all.  The bias/noise-parity theory predicts $L^* \propto (\omega\,dt)^{-2/3}$,
a different functional form from the observed pattern,
and the crossover occurs at lower $L$ than the theory predicts
because the relevant threshold is not directional MSE parity but
the point at which shared-arm confidence in a stale direction
outweighs the coverage diversity of independent agents —
a policy nonlinearity inaccessible to information-geometric
arguments.
