...
\begin{enumerate}
  \item We demonstrate and directly isolate a stale-coordination
  failure mechanism: confidence in a shared but stale direction
  eliminates the coverage diversity of independent agents, reversing
  the coordination benefit.  We verify this mechanistically with
  instrumented runs that simultaneously show a U-shaped error curve
  (minimum at $\Lmax$) and inter-agent angular dispersion collapsing
  to zero in the shared arm while the independent arm retains
  directional diversity (Figure~\ref{fig:dispersion}).
  This is well-supported in the slow rotation condition and observed,
  with less sharpness, in a second interior speed.
...
\subsection{Direct mechanism isolation}
...
\begin{enumerate}
  \item \textbf{Shared arm error is U-shaped, minimised at $\Lmax$.}
  ...
  angular lag $\omega(L{-}1)dt/2$ accumulates.  The minimum occurs
  at exactly $L{=}\Lmax{=}10$.  The independent arm's error
  decreases from 0.566 to 0.488~rad between $L{=}10$ and
...
The performance advantage of the independent arm at $L{>}\Lmax$
therefore has a direct directional cause: retained inter-agent
angular coverage, not individual noise reduction.  The mechanism
is not inferred from the performance gap — it is directly measured.
...
\caption{Direct mechanism isolation, $\omega{=}1.5$ (slow, $n{=}32$
seeds).
\textbf{(a)} Mean angular error $|\hat\theta-\theta_{\rm true}|$ vs.\
$L$.  Shared arm (blue) is U-shaped with minimum at $\Lmax{=}10$
(0.286~rad), then rises as predicted angular lag $\omega(L{-}1)dt/2$
(dotted) accumulates.  Independent arm (red) decreases monotonically.
\textbf{(b)} Inter-agent directional dispersion (circular std across
agents).  Shared arm is identically zero for all $L$ (all agents
share the same stale team estimate); independent arm retains 0.585~rad
at $L{=}10$ and 0.343~rad at $L{=}67$.
The coordination benefit of the shared arm collapses exactly where
agents converge on a direction that is confidently wrong.}
...
regime ($\Lmax/\Tcorr \approx 0.30$) and is the significance-based
threshold at two interior speeds.  The slow anchor ($p{=}0.049$)
does not survive uniform Holm correction, and the $n{=}64$
sensitivity analysis confirms the anchor after effect shrinkage.
...
The quantitative constant $c{\approx}0.30$ is primarily anchored on
the slow condition, where the two operationalisations agree.
