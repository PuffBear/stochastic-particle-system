...
beneficial.  The coordination benefit landscape under rotation is
\emph{regime-dependent}: a linear-rotation regime
($\omega\Lmax dt \approx 0.30\;\mathrm{rad}$, sinc attenuation
negligible) exhibits a clean significance-based crossover at
$\Lmax \approx 0.30\,\Tcorr$ at two interior speeds; a
sinc-attenuation regime ($\omega{\in}\{2.0,\,2.5\}$) dampens
the estimate amplitude across the tested $L$ range, suppressing the
sharp crossover; a rapid-rotation regime ($\omega{=}10.0$)
drives the predicted $\Lmax$ below integer resolution; and a
long-episode regime ($\omega{=}1.0$, $\Tcorr{\approx}H$) exhibits
...
of total field rotation at the threshold — the
...
The midpoint-lag formula $\hat\theta = \theta(t) - \omega L\,dt/2$
...
\paragraph{The regime taxonomy as a positive contribution.}
The non-conforming conditions ($\omega{\in}\{1.0, 2.0, 2.5, 10.0\}$) are not
random scatter — they have distinct, characterisable structure that constitutes
a second contribution of this work.  The sinc-attenuation regime
($\omega{\in}\{2.0, 2.5\}$) has a mechanistic account: amplitude damping
weakens the ``confident-wrong-direction'' mechanism proportionally across the
...
regime ($\omega\Lmax dt \approx 0.30\;\text{rad}$) and is
confirmed at two interior speeds; the borderline slow anchor
($p{=}0.049$) does not survive uniform Holm correction.
Four tested speeds fall into three other regimes (sinc-attenuation,
rapid-rotation, long-episode; see Sec.~\ref{sec:lmax-scaling}), each
with its own account of why a sharp crossover does not appear.
The central open question is what governs regime membership for a
given $(\omega, H)$ pair (Sec.~\ref{sec:discussion}).
...
We demonstrated a stale-coordination failure mechanism in which the
shared arm drives all collectors confidently toward a systematically
stale direction, eliminating the coverage diversity that makes
coordination beneficial.  The coordination benefit landscape under
...
MSE parity.
\emph{Sinc-attenuation regime} ($\omega{\in}\{2.0, 2.5\}$):
amplitude attenuation ($\mathrm{sinc}{\in}[0.59, 0.87]$ at large $L$)
...
\emph{Rapid-rotation regime} ($\omega{=}10.0$): predicted $\Lmax$
falls below integer resolution; the only nominally significant cell
($L{=}30$, $t{=}2.04$) is a false positive at 4.7\% estimate amplitude;
...
The $c \approx 0.30$ constant is a preliminary two-anchor result;
the borderline slow anchor ($p{=}0.049$) motivates further validation.
