...
Exploratory runs at $\omega{=}1.0$ ($\Tcorr{=}50$) and
$\omega{=}2.0$ ($\Tcorr{=}25$) reveal anomalies in both
directions.  At $\omega{=}1.0$, only $L{=}1$ is significant
($t{=}2.68$; all $L{\geq}3$ non-significant, $t{<}0.5$),
giving $c{=}0.02$ — far below $0.30$.  At $\omega{=}2.0$,
all eight $L$ levels remain significant (min.\ $t{=}1.93$ at
$L{=}45$), giving $c\geq2.68$ — far above $0.30$.
The $c{=}0.30$ relationship holds only at the original two interior
speeds ($\omega{\in}\{1.5,\,5.0\}$); whether it extends to other
rotation speeds is not established.
...
Exploratory runs at two further speeds do not conform: $\omega{=}1.0$
gives $\Lmax{=}1$ ($c{=}0.02$, anomalously low), and $\omega{=}2.0$
keeps all $L$ levels significant through the full episode ($c{\geq}2.68$,
anomalously high), as does $\omega{=}2.5$.
The constant $c$ is a preliminary two-speed observation, not a
universal scaling law.  The bias/noise-parity theory predicts $L^* \propto (\omega\,dt)^{-2/3}$,
