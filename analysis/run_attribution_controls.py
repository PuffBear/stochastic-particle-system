...
FROZEN_SEEDS = tuple(range(4001, 4009))
FROZEN_ALPHA = 0.06
...
def _source_snapshot(root: Path) -> dict[str, object]:
    paths = (
        root / "analysis" / "run_attribution_controls.py",
...
def evaluate_gate(summaries: list[dict[str, object]]) -> dict[str, object]:
    ...
    # Also compute oracle minus independent for reference
    ...
    primary_contrast = _descriptive_contrast(
        ...
        name="shared_minus_independent_at_alpha_0.06",
    )
    attr_check_1 = _descriptive_contrast(
        oracle_minus_shared,
        name="oracle_minus_shared_at_alpha_0.06",
    )
    attr_check_2 = _descriptive_contrast(
        shared_minus_stationary,
        name="shared_minus_stationary_at_alpha_0.06",
    )
...
def main() -> None:
    ...
    if frozen["diagnostic_seeds"] != list(FROZEN_SEEDS):
        raise RuntimeError("frozen config diagnostic_seeds differ from SPS-WO-07 constants")
    if frozen["alphas"] != list(FROZEN_ALPHAS):
        raise RuntimeError("frozen config alphas differ from SPS-WO-07 constants")
